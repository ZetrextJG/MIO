...
SDG_PARAMS = {
    ...
    "layers": [1, 2, 3, 4],
    "neurons": [10, 15, 30],
    "activation": [("relu", "xavier"), ("tanh", "he")],
}
...
def eval_sdg_model(X_train_scaled, y_train_scaled, X_test_scaled, **kwargs):
    ...
    train_losses, _ = trainer.train(EPOCHS)
...
def eval_rmsprop_model(X_train_scaled, y_train_scaled, X_test_scaled, **kwargs):
    model = mc.SimpleDense(
        1,
        1,
        kwargs["layers"],
    ...
    optimizer = RMSProp(
        model.parameters(), learning_rate=kwargs["learning_rate"], beta=kwargs["beta"]
    )
    loss = MeanSquareErrorLoss()
    train_dataloader = NumpyRegressionDataloader(
        X_train_scaled, y_train_scaled, batch_size=kwargs["batch_size"], shuffle=True
...
def main():
    for train_file, test_file in DATASETS:
        ...
        results = []
        output_file_rmsprop = Path(
            f"search_results_rmsprop_{train_file.split('/')[-1]}.jsonl"
        )
        for params in tqdm(ParameterGrid(RMSPROP_PARAMS)):
            train_losses, y_test_pred = eval_rmsprop_model(
                X_train_scaled, y_train_scaled, X_test_scaled, **params
            )
            y_test_pred = output_normalizer.reverse(y_test_pred)
            mse = mf.mse(y_test_pred, y_test)
        ...
        result_df = pd.DataFrame(results)
        result_df.to_json(output_file_rmsprop, orient="records", lines=True)
